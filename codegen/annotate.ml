...
let annotate_array_view
  (ctx : context)
  (av : Air.array_view) : A_air.buffer_info =
  let open A_air in
...
and annotate_par_stmt (ctx : context) (stmt : Air.par_stmt) : context =
  ...
  | Air.Parallel (dest, id, tavs, body) ->
      let buffer_infos = List.map tavs ~f:(fun (_, av) -> annotate_array_view ctx av) in
...
and annotate_seq_stmt_stmt
  (kernel_ctx : kernel_context)
  (ctx : context)
  (stmt : Air.seq_stmt Air.stmt) : kernel_context * context =
  match stmt with
...
and annotate_seq_stmt
  ...
  let (kernel_ctx', ctx) = match stmt with
    ...
    | Air.Seq_stmt seq_stmt ->
        annotate_seq_stmt_stmt kernel_ctx ctx seq_stmt
  in
  ...
  let kernel_ctx =
    { used = Set.union kernel_ctx.used kernel_ctx'.used;
      ...
      additional_buffers = Map.merge_skewed kernel_ctx.additional_buffers kernel_ctx.additional_buffers
        ~combine:(fun ~key -> failwith "Duplicate key.")
    }
  in
